...
combine([C1, C2 | Cs]) ->
    ...
    EVars = sets:union(C1#constraints.exist_vars, C2#constraints.exist_vars),
...
solve(Constraints, Env) ->
    ...
    Subst = { maps:map(GlbSubs, maps:with(sets:to_list(ElimVars), Cs#constraints.upper_bounds)),
              maps:map(LubSubst, maps:with(sets:to_list(ElimVars), Cs#constraints.lower_bounds)) },
    UBounds = maps:without(sets:to_list(ElimVars), Cs#constraints.upper_bounds),
    LBounds = maps:without(sets:to_list(ElimVars), Cs#constraints.lower_bounds),
