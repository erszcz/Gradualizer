...
%                                                    remove_pos(P)
%                                            end, Params)};
remove_pos({type, _, 'fun', _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, any, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, arity, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, atom, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, binary, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, bitstring, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, boolean, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, bounded_fun, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, byte, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, char, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, float, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, function, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, integer, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, iodata, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, iolist, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, list, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, map, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, maybe_improper_list, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, mfa, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, module, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, neg_integer, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, nil, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, no_return, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, node, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, non_neg_integer, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, none, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, nonempty_improper_list, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, nonempty_list, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, nonempty_maybe_improper_list, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, number, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, pid, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, port, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, pos_integer, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, product, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, range, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, reference, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, string, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, term, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, timeout, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, tuple, _} = Type) ->
    remove_pos_builtin_type(Type);
remove_pos({type, _, union, _} = Type) ->
    remove_pos_builtin_type(Type);
...
remove_pos_builtin_type({type, _, Type, Params}) ->
    {type, erl_anno:new(0), Type, lists:map(fun
                                                ({type, _, any}) ->
                                                    {type, erl_anno:new(0), any};
                                                (P) when is_list(P) ->
