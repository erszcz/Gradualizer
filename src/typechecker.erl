...
normalize_rec({type, Ann, map, Assocs}, TEnv, _Unfolded) when is_list(Assocs) ->
    {type, Ann, map, [normalize(As, TEnv) || As <- Assocs]};
normalize_rec({type, Ann, Assoc, KeyVal}, TEnv, _Unfolded)
...
-spec mta(_, tenv()) -> ok.
mta({user_type, Anno, Name, Args}, TEnv) ->
