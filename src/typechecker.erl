...
stop_now_or(Default, Ty, Trace, Fun) ->
    case stop_recursion(RefinableTy, Trace) of
        stop -> Default;
        {proceed, NewTrace} -> Fun(NewTrace)
    end.

normalize({type, _, record, [{atom, _, Name}|Fields]} = Ty, TEnv, Trace) when length(Fields) > 0 ->
    ProceedF = fun (NewTrace) ->
                       NormFields = [type_field_type(FieldName, normalize(Type, TEnv, Trace))
                                     || ?type_field_type(FieldName, Type) <- Fields],
                       type_record(Name, NormFields)
               end
    stop_or_proceed(Ty, Ty, Trace, ProceedF);
normalize({type, _, union, Tys}, TEnv, Trace) ->
    Types = flatten_unions(Tys, TEnv, Trace),
    case merge_union_types(Types, TEnv, Trace) of
        []  -> type(none);
        [T] -> T;
...
normalize({remote_type, P, [{atom, _, M}, {atom, _, N}, Args]}, TEnv, Trace) ->
    case gradualizer_db:get_exported_type(M, N, Args) of
        {ok, T} ->
            normalize(typelib:remove_pos(T), TEnv, Trace);
        opaque ->
            NormalizedArgs = lists:map(fun (Ty) -> normalize(Ty, TEnv, Trace) end, Args),
...
expect_record_type(?top() = _TermTy, _Record, _TEnv) ->
    any;
...
type_check_comprehension(Env, Compr, Expr, [{b_generate, _P, Pat, Gen} | Quals]) ->
    ...
    {_PatTys, _UBounds, NewVEnv, Cs2} =
        add_types_pats([Pat], [BitStringTy], Env#env.tenv, Env#env.venv, capture_vars),
    {TyL, VarBinds2, Cs3} =
...
do_type_check_expr_in(Env, Ty, {match, _, Pat, Expr}) ->
    ...
    {_PatTys, _UBounds, NewVEnv, Cs2} =
        add_types_pats([Pat], [Ty], Env#env.tenv, Env#env.venv, capture_vars),
...
type_check_comprehension_in(Env, ResTy, OrigExpr, Compr, Expr, P,
    ...
    {_PatTys, _UBounds, NewVEnv, Cs2} =
        add_types_pats([Pat], [BitTy], Env#env.tenv, Env#env.venv, capture_vars),
    NewEnv = Env#env{venv = NewVEnv},
...
refine_mismatch_using_guards(PatTys, {clause, _, _, _, _}, _VEnv, _TEnv) ->
    %% No Refinement
    PatTys.
...
refinable(?type(Name, Tys) = Ty0, TEnv, Trace)
  when (tuple =:= Name orelse union =:= Name)
   and is_list(Tys) ->
    case stop_recursion(Ty0, Trace) of
        stop ->
            true;
        {proceed, NewTrace} ->
...
refinable(?type(map, _) = Ty0, TEnv, Trace) ->
    ...
    case stop_recursion(Ty, Trace) of
        stop -> true;
        {proceed, NewTrace} ->
...
-spec stop_recursion(_, _) -> stop | {proceed, sets:set()}.
stop_recursion(RefinableTy, Trace) ->
...
add_type_pat(Lit = {Tag, P, Val}, Ty, TEnv, VEnv)
  when Tag =:= integer;
       Tag =:= char ->
...
add_type_pat(Tuple = {tuple, P, Pats}, Ty, TEnv, VEnv) ->
    case expect_tuple_type(Ty, length(Pats)) of
        any ->
            ...
            ,constraints:empty()};
        {elem_ty, Tys, Cs} ->
            {PatTys, UBounds, VEnv1, Cs1} = do_add_types_pats(Pats, Tys, TEnv, VEnv),
            {type(tuple, PatTys)
            ,type(tuple, UBounds)
            ,VEnv1
